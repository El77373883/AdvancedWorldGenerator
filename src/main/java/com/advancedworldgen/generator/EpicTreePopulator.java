...
public class EpicTreePopulator extends BlockPopulator {
    ...
    // Arbol Amazonas GIGANTE 40-50 bloques
            ...
            for (int dz = -6; dz <= 6; dz++) {
                if (rand.nextInt(3) == 0) {
                    ...
                    for (int dy = 1; dy <= lianaLen; dy++) {
                        ...
                        if (r.isInRegion(lx, ly, lz) &&
                            (r.getType(lx, ly, lz) == Material.AIR ||
                             r.getType(lx, ly, lz) == Material.CAVE_AIR)) {
                            setBlock(r, lx, ly, lz, Material.VINE);
                        } else break;
                    }
                }
            }
    ...
    // Necesario para que Block funcione en getSurfaceY
    private Block getBlock(LimitedRegion r, int x, int y, int z) {
        return r.getBlockAt(x, y, z);
    }
}
