...
public class FrozenRealmGenerator extends ChunkGenerator {
    ...
    public void generateSurface(WorldInfo worldInfo, Random random, int chunkX, int chunkZ, ChunkData chunkData) {
        ...
        NoiseUtil glacier = new NoiseUtil(seed + 7000);
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                double mountH = mount.octaveNoise(wx / 140.0, wz / 140.0, 5, 0.65, 2.3);
                double det = detail.octaveNoise(wx / 45.0, wz / 45.0, 3, 0.4, 2.0);
                double glacierH = glacier.octaveNoise(wx / 100.0, wz / 100.0, 4, 0.5, 2.0);

                double mf = Math.max(0, mountH);
                mf = Math.pow(mf, 1.4);

                // Glaciares y montañas heladas enormes
                double heightD = 64 + baseH * 30 + mf * 140 + det * 8 + glacierH * 25;
                ...
                for (int y = -59; y < height; y++) {
                    ...
                    if (!isCave) {
                        ...
                        else if (y < 0) chunkData.setBlock(x, y, z, Material.STONE);
                        else if (y < 80) chunkData.setBlock(x, y, z, random.nextInt(8) == 0 ? Material.PACKED_ICE : Material.STONE);
                        else chunkData.setBlock(x, y, z, random.nextInt(5) == 0 ? Material.BLUE_ICE : Material.PACKED_ICE);
                    }
                }

                for (int y = height - 3; y < height; y++)
                    if (y >= -64) chunkData.setBlock(x, y, z, Material.SNOW_BLOCK);

                if (height >= 0) chunkData.setBlock(x, height, z, Material.SNOW_BLOCK);

                // Lagos de hielo
                if (height < 62) {
                    for (int y = height + 1; y <= 62; y++) chunkData.setBlock(x, y, z, Material.ICE);
                    chunkData.setBlock(x, height, z, Material.SNOW_BLOCK);
                }
            }
        }
    }
    ...
    public BiomeProvider getDefaultBiomeProvider(WorldInfo worldInfo) {
        return new FrozenBiomeProvider(worldInfo.getSeed());
    }

    @Override
    public List<BlockPopulator> getDefaultPopulators(World world) {
        List<BlockPopulator> p = new ArrayList<>();
        p.add(new FrozenTreePopulator());
