...
public class DuneWorldGenerator extends ChunkGenerator {
    ...
    public void generateSurface(WorldInfo worldInfo, Random random, int chunkX, int chunkZ, ChunkData chunkData) {
        ...
        NoiseUtil dune = new NoiseUtil(seed + 5000);
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                ...
                double baseH = base.octaveNoise(wx / 180.0, wz / 180.0, 6, 0.55, 2.1);
                double mountH = mount.octaveNoise(wx / 130.0, wz / 130.0, 5, 0.65, 2.3);
                double det = detail.octaveNoise(wx / 40.0, wz / 40.0, 3, 0.4, 2.0);
                double duneH = dune.octaveNoise(wx / 60.0, wz / 60.0, 4, 0.5, 2.0);

                double mf = Math.max(0, mountH);
                mf = Math.pow(mf, 1.4);

                // Montañas grandotas epicas
                double heightD = 64 + baseH * 35 + mf * 160 + det * 12 + duneH * 20;
                ...
                for (int y = -59; y < height; y++) {
                    ...
                    if (!isCave) {
                        if (y < -48) chunkData.setBlock(x, y, z, random.nextInt(15) == 0 ? Material.DEEPSLATE_DIAMOND_ORE : Material.DEEPSLATE);
                        else if (y < -30) chunkData.setBlock(x, y, z, random.nextInt(25) == 0 ? Material.DEEPSLATE_GOLD_ORE : Material.DEEPSLATE);
                        else if (y < 0) chunkData.setBlock(x, y, z, random.nextInt(20) == 0 ? Material.IRON_ORE : Material.SANDSTONE);
                        else chunkData.setBlock(x, y, z, Material.SANDSTONE);
                    }
                }

                for (int y = height - 3; y < height; y++)
                    if (y >= -64) chunkData.setBlock(x, y, z, Material.SAND);

                if (height >= 0) {
                    Material top = height > 180 ? Material.SANDSTONE : Material.SAND;
