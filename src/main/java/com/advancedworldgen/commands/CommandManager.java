...
public class CommandManager implements CommandExecutor, TabCompleter {
    ...
    private void handleCreate(CommandSender sender, String[] args) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            // ✅ CORREGIDO: .get() en vez de .join()
            World world;
            try {
                world = Bukkit.getScheduler().callSyncMethod(plugin, creator::createWorld).get();
            } catch (Exception e) {
                world = null;
            }
            ...
            Bukkit.getScheduler().runTask(plugin, () -> {
                if (finalWorld != null) {
                    finalWorld.setDifficulty(Difficulty.NORMAL);
                    finalWorld.setSpawnFlags(true, true);
                    finalWorld.setTime(6000);
